// Generated by scripts/generate-contracts.mjs. Do not edit.

export type AgentState = 'idle' | 'listening' | 'recognizing' | 'thinking' | 'confirming' | 'acting' | 'speaking' | 'error';
export type AgentEventType = 'client.message' | 'agent.state' | 'assistant.delta' | 'assistant.message' | 'tool.confirmation_required' | 'tool.result' | 'request.cancel' | 'request.cancelled' | 'error';
export type ToolRiskLevel = 'L0' | 'L1' | 'L2' | 'L3';

export interface AgentEvent<TData extends Record<string, unknown> = Record<string, unknown>> {
  type: AgentEventType;
  version: '1.0';
  session_id: string;
  request_id: string;
  timestamp: string;
  data: TData;
}

export interface AgentError {
  error_code: string;
  message: string;
  recoverable: boolean;
  request_id: string;
  details?: Record<string, unknown>;
}
